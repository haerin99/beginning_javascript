...
// If document does not contain such a section, create one.
function debug(msg) {
    ...
    // Now wrap message in its own <pre> and append it to the log
    var pre = document.createElement("pre");   // Create <pre> tag
    var text = document.createTextNode(msg);   // Wrap msg in a text node
    pre.appendChild(text);    // Add text to <pre>
    log.appendChild(pre);     // Add <pre> to log
}
